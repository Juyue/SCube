...
def main():
    ...
    config_coarse = "configs/fvdb_example_data/train_diffusion_16x16x16_dense.yaml"
    ckpt_coarse = "checkpoints/fvdb_example_data/coarse_diffusion/last.ckpt"

    config_fine = "configs/fvdb_example_data/train_diffusion_128x128x128_sparse.yaml"
    ckpt_fine = "checkpoints/fvdb_example_data/fine_diffusion/last.ckpt"
    ...
    logger.info(f"Sampling from scube on {known_args.category} ...")
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            for batch_idx in range(output_x.grid.grid_count):
                ...
                result_dict['coarse_normal'] = res_coarse.normal_features[-1].data[batch_idx].jdata.cpu().numpy() 
                ## fine stage
                result_dict['fine_xyz'] = output_x.grid.grid_to_world(output_x.grid[batch_idx].ijk.float()).jdata.cpu().numpy()
                result_dict['fine_normal'] = res.normal_features[-1].data[batch_idx].jdata.cpu().numpy()
                ...
                if known_args.extract_mesh:
                    ...
                    pd_normal = res.normal_features[-1].data[batch_idx].jdata
                    with torch.no_grad():
