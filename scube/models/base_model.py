...
from scube.utils.loss_util import AverageMeter
...
class BaseModel(pl.LightningModule):
    ...
    def load_module(spec_path, weight_path=None, overwrite_config=None):
        ...
        basis_net_module = importlib.import_module("scube.models." + config_args.model).Model
    ...
    def train_dataloader(self):
        ...
        torch.manual_seed(0)
        return DataLoader(train_set, batch_size=self.hparams.batch_size // self.trainer.world_size, 
    ...
    def generate_fvdb_grid_on_the_fly(batch):
        if DS.INPUT_PC_RAW in batch and DS.INPUT_PC in batch and not isinstance(batch[DS.INPUT_PC], fvdb.GridBatch):
    ...
    def generate_latent_semantic_on_the_fly(batch, latent_grid):
        ...
        for i in range(B):
            ...
            input_pc_xyz = batch[DS.INPUT_PC][i].grid_to_world(batch[DS.INPUT_PC][i].ijk.float()).jdata
            input_semantic = batch[DS.GT_SEMANTIC][i]

            latent_semantic = semantic_from_points(latent_xyz, input_pc_xyz, input_semantic)
