...
from scube.modules.diffusionmodules.openaimodel.util import (
    checkpoint,
    conv_nd,
    linear,
    avg_pool_nd,
...
from scube.modules.diffusionmodules.openaimodel.attention_dense import SpatialTransformer
...
class QKVAttention(nn.Module):
    ...
    def forward(self, qkv):
        ...
        q, k, v = qkv.chunk(3, dim=1) # (bs, 3 * n_heads * ch, length) -> (bs, n_heads * ch, length)
        q, k, v = map(lambda t: rearrange(t, 'b (h d) n -> b h n d', h=self.n_heads).contiguous(), (q, k, v))
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if legacy:
            #num_heads = 1
            dim_head = ch // num_heads if (use_spatial_transformer or middile_force_spatial_transformer) else num_head_channels
        self.middle_block = TimestepEmbedSequential(
            ...
            AttentionBlock(
                ...
                num_heads=num_heads,
                num_head_channels=dim_head,
            ) if (not use_spatial_transformer) and (not middile_force_spatial_transformer) else 
            SpatialTransformer(
                ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim, disable_sa=middle_disable_first_sa
            ),
            ResBlock(
                ch,
                time_embed_dim,
