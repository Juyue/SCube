...
from scube.utils.loss_util import TorchLossMeter
...
class Loss(nn.Module):
    ...
    def cross_entropy(self, pd_struct: fvnn.VDBTensor, gt_grid: fvdb.GridBatch, dynamic_grid: fvdb.GridBatch = None):
        assert torch.allclose(pd_struct.grid.origins, gt_grid.origins, atol=1e-7), f"{pd_struct.grid.origins} != {gt_grid.origins}"
        assert torch.allclose(pd_struct.grid.voxel_sizes, gt_grid.voxel_sizes, atol=1e-7), f"{pd_struct.grid.voxel_sizes} != {gt_grid.voxel_sizes}"
        idx_mask = gt_grid.ijk_to_index(pd_struct.grid.ijk).jdata == -1 # 0 is occupied, 1 is empty
        idx_mask = idx_mask.long()

        if self.hparams.supervision.adaptive_structure_weight:
            # min(max(#N/#P, 1), 100)
            positive_weight = min(max(1, idx_mask.shape[0] / (torch.sum(idx_mask == 0) + 1e-6).item()), 100)
        ...
        if dynamic_grid is not None:
            ...
            loss = F.cross_entropy(pd_struct.data.jdata, idx_mask, weight=weight, reduction='none') * dynamic_mask.float()
            loss = loss.mean()
        else:
